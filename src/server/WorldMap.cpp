...
bool regionsCmp(const Region *r1, const Region *r2)
{
	return r1->n_pls < r2->n_pls;
}

void WorldMap::balance_lightest()
{
	int lightload_thres = n_players / 4 * 1, maxload_thres = n_players / 4 * 1.2;
	int safeload_thres = (lightload_thres + maxload_thres) / 2;
	printf("safe %d, max %d, light %d\n", safeload_thres, maxload_thres, lightload_thres);
	bool is_loadbalanced = true;


	while(is_loadbalanced)
	{
		int max_tid = -1, min_tid = -1, max_load = -1, min_load = n_players;
		vector<Region *> maxload_regions;
		is_loadbalanced = false;

		for(int i = 0; i < sd->num_threads; i++)
		{
			if(players[i].size() > maxload_thres && max_load < players[i].size())
			{
				max_tid = i;
				max_load = players[i].size();
			}
			if(players[i].size() < maxload_thres && min_load > players[i].size())
			{
				min_tid = i;
				min_load = players[i].size();
			}
		}

		if(max_tid == -1 || min_tid == -1) return;

		printf("*******Before load balance***************\n");
		for(int i = 0; i < sd->num_threads; i++) printf("thread %d: players num: %d\n", i, players[i].size());
		printf("*******End*********************\n");

		for(unsigned int x = 0; x < n_regs.x; x++)
			for(unsigned int y = 0; y < n_regs.y; y++)
			{
				printf("%d ", regions[x][y].n_pls);
				if(regions[x][y].layout == max_tid)
				{
					maxload_regions.push_back(&regions[x][y]);
				}
			}

		sort(maxload_regions.begin(), maxload_regions.end(), regionsCmp);

		printf("sorted regions: ");
		for(int i = 0; i < maxload_regions.size(); i++) printf("%u ", maxload_regions[i]->n_pls);
		printf("\n");


		printf("start load balance...\n");
		for(int i = 0; i < maxload_regions.size() && max_load - maxload_regions[i]->n_pls > min_load + maxload_regions[i]->n_pls; i++)
		{
			if(max_load > safeload_thres && min_load + maxload_regions[i]->n_pls < safeload_thres && maxload_regions[i]->n_pls != 0)
			{
				is_loadbalanced = true;
				reassignRegion(maxload_regions[i], min_tid);
				printf("after reassign: maxload: %d, lightload: %d\n", players[max_tid].size(), players[min_tid].size());
				min_load += maxload_regions[i]->n_pls;
				max_load -= maxload_regions[i]->n_pls;	
			}
